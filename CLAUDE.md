...
- Required environment variables for full functionality:
  ...
  - `SPOTIFY_OWNER_USER_ID` - Your Spotify user id (from `GET /v1/me`). The `/api/auth/spotify/callback` route refuses to save tokens unless the freshly-authorized Spotify account matches this id, and `scripts/syncPlaylists.ts` uses it to filter `/me/playlists` down to your own playlists. Access to `/api/auth/spotify/start` itself is gated by the Google-owner session (same as `/admin`).
  ...
  - `NEXT_PUBLIC_UMAMI_WEBSITE_ID` - Optional, build-time only. Baked into the image by `.github/workflows/deploy.yml` from the `UMAMI_WEBSITE_ID` GHA repo variable (`gh variable set UMAMI_WEBSITE_ID`). When set, the root layout mounts the self-hosted Umami tracker at `/_/i.js` (Caddy on the droplet reverse-proxies `/_/i.js` → Umami's `/i.js` and `/_/e` → Umami's pageview endpoint, both over Tailscale to `media:3001` on the mini PC). Same-origin obscure paths defeat ad-blocker filter lists. Unset ⇒ no tracker mounted — this is the correct default for local dev and for `docker build` without a build-arg.
